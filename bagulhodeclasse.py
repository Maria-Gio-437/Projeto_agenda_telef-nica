...
class Agenda:
    ...
    def template(self):
        ...
        dados_no_terminal.mostra_dados()
    ...
    def frames(self):
        ...
        self.frame_base = tk.Frame(self.frame2, bg='mintcream', border=2,
            highlightbackground='gray25', highlightthickness=4)
        self.frame_base.place(x=20, y=70, height=430, width=410)

        self.frame_lista = tk.Frame(self.frame_base, bg='mintcream', border=2,
            highlightbackground='gray25', highlightthickness=4)
        self.frame_lista.place(x=-6, y=-6, height=430, width=410)

        self.frame_modificar = tk.Frame(self.frame_base, bg='mintcream',
            border=2, highlightbackground='gray25', highlightthickness=4)
        self.frame_modificar.place(x=-6, y=-6, height=430, width=410)


    def labels(self):
        self.texto_saudacao = tk.Label(self.frame0, font=['Arial', 20, 'bold'],
            text='AGENDA TELEFÔNICA', bg='lightsteelblue', fg='gray12')
        self.texto_saudacao.place(x=45, y=30)

        self.texto_nome = tk.Label(self.janela, text='Nome', bg='azure2',
            font=['Helvetica', 14])
        self.texto_nome.place(x=60, y=155)

        self.texto_telefone = tk.Label(self.janela, text='Telefone',
            bg='azure2', font=['Helvetica', 14])
        self.texto_telefone.place(x=60, y=250)

        self.texto_nome_modifica = tk.Label(self.frame_modificar,
            text='Selecione o nome:', bg='mintcream', font=['Helvetica', 14])
        self.texto_nome_modifica.place(x=10, y=10)

        self.texto_novo_nome_modifica = tk.Label(self.frame_modificar,
            text='Novo nome:', bg='mintcream', font=['Helvetica', 14])
        self.texto_novo_nome_modifica.place(x=10, y=50)

        self.texto_novo_cont_modifica = tk.Label(self.frame_modificar,
            text='Novo telefone:', bg='mintcream', font=['Helvetica', 14])
    ...
    def entradas(self):
        ...
        self.nome_modifica = tk.Entry(self.frame_modificar,
            font=['Calibri', 14], border=1)
        self.nome_modifica.place(x=180, y=10, height=30, width=200)

        self.novo_nome_modifica = tk.Entry(self.frame_modificar,
            font=['Calibri', 14], border=1)
        self.novo_nome_modifica.place(x=120, y=55, height=30, width=260)

        self.cont_modifica = tk.Entry(self.frame_modificar,
            font=['Calibri', 14], border=1)
        self.cont_modifica.place(x=150, y=100, height=30, width=200)

        self.cont2_modifica = tk.Entry(self.frame_modificar,
            font=['Calibri', 14], border=1)
        self.cont2_modifica.place(x=150, y=150, height=30, width=200)

        self.cont3_modifica = tk.Entry(self.frame_modificar,
            font=['Calibri', 14], border=1)
        self.cont3_modifica.place(x=150, y=200)


    def botao(self):
        self.adicionar = tk.Button(self.janela, text='Adicionar', border=2,
            bg='gray25', command=self.conecta_Bd)
        self.adicionar.place(x=320, y=480, height=30, width=80)

        self.lista = tk.Button(self.janela, text='Contatos salvos',
            font=['Calibri', 15], bg='gray25', border=0, command=self.tabela)
        self.lista.place(x=520, y=50, height=35, width=200)

        self.modificar = tk.Button(self.janela, text='Modificar contatos',
            font=['Calibri', 15], bg='gray25', border=0,
            command=self.destruir_frame_lista)
        self.modificar.place(x=730, y=50, height=35, width=200)

        self.deletar_dados = tk.Button(self.frame_modificar, text='Deletar',
            border=2, bg='gray25', command=self.deletar)
        self.deletar_dados.place(x=150, y=300, height=30, width=80)

        self.alterar_dados = tk.Button(self.frame_modificar, text='Alterar',
            border=2, bg='gray25', command=self.alterar)
    ...
    def conecta_Bd(self):
        ...
        self.cursor.execute(f'''INSERT INTO agenda_telefonica(Nome,
            Tel_1, Tel_2, Tel_3) VALUES ('{self.valor1}',' {self.valor2}',
            '{self.valor3}', '{self.valor4}')''')
    ...
    def tabela(self):
        ...
        self.tabela_list = ttk.Treeview(self.frame_lista,
            columns=['col_nome', 'col_cont', 'col_cont2', 'col_cont3'],
            show='headings')
        self.tabela_list.column('col_nome', minwidth=0, width=87)
        self.tabela_list.column('col_cont', minwidth=0, width=87)
        self.tabela_list.column('col_cont2', minwidth=0, width=87)
        self.tabela_list.column('col_cont3', minwidth=0, width=87)
        self.tabela_list.heading('col_nome', text='Nome')
        self.tabela_list.heading('col_cont', text='Telefone')
        self.tabela_list.heading('col_cont2', text='Telefone 2')
        self.tabela_list.heading('col_cont3', text='Telefone 3')

        self.conexao = sqlite3.connect('mariagio.db')
        self.cursor = self.conexao.cursor()

        #mostra dados na tabela
        mostrar = self.cursor.execute('''SELECT Nome,
            Tel_1, Tel_2, Tel_3 FROM agenda_telefonica''')
        self.conexao.commit()
        c = 0
        for i in mostrar:
            self.tabela_list.insert(parent='', index='end', iid=c, text='poha',
                values=(i))
            c += 1

        self.tabela_list.place(x=-3, y=-3, height=423, width=385)

        self.rolagem = ttk.Scrollbar(self.frame_lista, orient='vertical')
        self.tabela_list.configure(yscroll=self.rolagem.set)
    ...
    def destruir_frame_lista(self):
        ...
        self.deletar_dados = tk.Button(self.frame_modificar,
            text='Deletar', border=2, bg='gray25', command=self.deletar)
        self.deletar_dados.place(x=150, y=300, height=30, width=80)
        self.alterar_dados = tk.Button(self.frame_modificar, text='Alterar',
            border=2, bg='gray25', command=self.alterar)
        self.alterar_dados.place(x=280, y=300, height=30, width=80)
    ...
    def deletar(self):
        ...
        self.cursor.execute(f''' DELETE FROM agenda_telefonica
            WHERE Nome='{self.nome_excluido}' ''')
        self.conexao.commit()
    ...
    def alterar(self):
        ...
        self.cursor.execute(f''' UPDATE agenda_telefonica
                                 SET Nome='{self.nome_alterado}', Tel_1='{self.tel1_alterado}',
                                 Tel_2='{self.tel2_alterado}', Tel_3='{self.tel3_alterado}' 
                                 WHERE Nome='{self.nome_tabela}' ''')
        self.conexao.commit()
